...
function DecreaseCustomerCartQty(ProductPricingId) {
    // Get the field name
    var quantity = parseInt($("#quantity_" + ProductPricingId).val());
    ...
    if (quantity > 1) {
        $("#quantity_" + ProductPricingId).val(quantity - 1);
        UpdateCustomerCartItem(ProductPricingId);
    } else {
        DeleteCustomerCartItem(ProductPricingId);
    }

}

function IncreaseCustomerCartQty(ProductPricingId) {
    // Get the field name
    var quantity = parseInt($("#quantity_" + ProductPricingId).val());
...
function UpdateCustomerCartItem(ProductPricingId) {
    var CartItemQuantity = $("#quantity_" + ProductPricingId).val();
    //$("#AddToCartModal").modal('toggle');
    $.ajax({
        url: '../Customer/UpdateCartItem',
        type: 'POST',
        data: { ProductPricingId: ProductPricingId, CartItemQuantity: CartItemQuantity },
        success: function (data) {
...
function DeleteCustomerCartItem(ProductPricingId) {
    $.ajax({
...
function GetCustomerCartCount() {
    $.ajax({
        url: '../Customer/GetCartCount',
        type: 'GET',
        success: function (data) {
            var cartElement = $("#Cart");
            var count = data;
